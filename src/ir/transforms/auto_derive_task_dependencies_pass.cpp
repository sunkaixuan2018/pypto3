...
bool IsCompilerAutoManualScopeTargetCall(const CallPtr& call) {
  return call && call->op_ && call->op_->name_.find("rope_kv_cache") != std::string::npos;
}
 ...
 protected:
  ...
  ExprPtr AnalyzeCallLike(const CallPtr& call, const Expr* identity_key,
    ...
    if (!compiler_edges.empty()) {
      ...
      if (!in_manual_scope && IsCompilerAutoManualScopeTargetCall(call)) {
