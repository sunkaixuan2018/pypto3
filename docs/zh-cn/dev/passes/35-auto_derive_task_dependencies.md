...
本 pass 会分析 MANUAL 和 AUTO 区域。手写的 AUTO `RuntimeScopeStmt` 在输出
IR 中仍保持 `manual=false`。对于默认 `auto_scope=True` 的 orchestration
函数，本 pass 运行在 `MaterializeRuntimeScopes` 之前，因此会把整个函数体当作
analysis-only 的虚拟 AUTO 区域来分析，但不会插入或移动 scope wrapper。
codegen 仍由 `MaterializeRuntimeScopes` 产生的 scope 发出 `PTO2_SCOPE()`，
runtime OverlapMap/TensorMap tracking 也继续启用。静态可表达的编译器推导边会
通过 `Arg::set_dependencies(...)` 叠加发出。
