...
如果某个 hazard 需要的 prior producer 没有静态绑定 TaskId，pass 会把整个 enclosing
`RuntimeScopeStmt` 改写为 `manual=false`。这样整段区域都会回到 runtime
OverlapMap/TensorMap tracking，而不会在 manual-scope 边界混用部分 compiler deps 与
runtime 状态。
