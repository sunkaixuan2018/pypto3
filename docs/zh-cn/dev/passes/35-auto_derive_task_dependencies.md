...
依赖边，本 pass 不会新增 compiler deps，也不会把 scope 降级为 AUTO。但 pass 仍可
检查这些显式 deps，并把已经被完整覆盖的只读 tensor 输入改成 `NoDep`。AUTO 区域只有在
...
9. 对静态证明 disjoint 的 region 跳过依赖边。否则，对 RAW、WAR、WAW hazard 从先前
   ...
   不会重复添加。
10. 对只读 `Input` 参数，如果它的精确静态 region（`Full` 或 `Box`）或已知 root 的
    conservative region 的 RAW hazard，已经被同一个成功分析的 runtime scope 内的用户边
    或编译器边覆盖，则把 call-site direction 改成 `NoDep`，让 codegen 生成
    `add_no_dep(...)`。在 MANUAL scope 中只使用用户手写 deps，不会合成 compiler edge。
...
MANUAL scope，因为 pass 在这里不合成 compiler deps；无法解析的 manual-scope hazard
只会阻止对应输入改成 `NoDep`。

## 默认路径变化

- MANUAL scope 不会获得编译器推导出的依赖边。用户手写的 `deps=[...]` 是
  `pl.manual_scope()` 内唯一的依赖来源，scope 保持 MANUAL。如果这些 deps 完整覆盖
  某个完整只读输入的 producer fan-in，该输入仍可以改成 `NoDep`。第一版不会在
  MANUAL scope 中应用 `InOut -> OutputExisting`。
...
也只做 verifier 合法的 direction 降级，并且对应 tensor slot 已经有显式依赖边兜底：
对覆盖完整的只读输入做 `Input -> NoDep`，对覆盖完整的 AUTO 读写 slot 做
`InOut -> OutputExisting`。
